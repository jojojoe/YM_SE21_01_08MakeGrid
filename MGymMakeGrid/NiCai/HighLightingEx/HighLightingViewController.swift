...
class HighLightingViewController: UIViewController{
    ...
    override func viewDidLayoutSubviews() {
        ...
        self.webView.frame = CGRect(x: 0, y: self.view.safeAreaInsets.top, width: self.view.width, height: self.view.safeAreaLayoutGuide.layoutFrame.height)
    }
...
extension HighLightingViewController {
    func actionForWeb(body:[String: Any]) {
        ...
        case .InAppBuy:
            guard let productId = params?[HightingParamKey.productId.rawValue] as? String else {
                return
            }
            inAppBuyAction(productId:productId, complete: { [weak self](done,receiptString) in
